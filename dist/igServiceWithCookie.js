...
export async function getInstagramDataByCookie(inputUrl, opts = {}) {
    ...
    if (!cookie) {
        return {
            ok: false,
            status: 400,
            error: "Missing cookie. Set env COOKIE or pass `cookieOverride`/`x-ig-cookie` header.",
        };
    }
    ...
    if (items?.product_type === "carousel_container" &&
        ...
        carousel_media = items.carousel_media.map((el) => ({
    ...
    const result = {
        ...
        comment_count: items?.comment_count ?? items?.edge_media_to_parent_comment?.count,
        ...
        image_versions: items?.image_versions2?.candidates ?? items?.display_resources,
